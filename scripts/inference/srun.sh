...
#   sbatch scripts/inference/srun.sh  # Auto-uses config.yaml if present
#
# Notes:
...
# Only match known sbatch options, not all --* arguments (to allow inference args)
SBATCH_OPTS=""
SCRIPT_ARGS=""
HAS_CONFIG=0
for arg in "$@"; do
...
if command -v srun &> /dev/null && [[ -n "${SLURM_JOB_ID:-}" ]]; then
    srun bash "${PROJECT_DIR}/scripts/inference/run.sh" ${SCRIPT_ARGS}
else
    bash "${PROJECT_DIR}/scripts/inference/run.sh" ${SCRIPT_ARGS}
fi
